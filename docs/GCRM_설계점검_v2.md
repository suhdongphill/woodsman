...
### P1 (2026-09-20) — 셋 중 **재정 우위만** 넘길 수 있었다

세 기둥을 다 올리려 했지만, 축별 커버리지를 계산해 보니 **넘길 수 있는 것은 하나뿐**이었다.
(계산은 설정만으로 잰 「천장」이다 — 자료가 다 있다고 쳤을 때의 값. ⭐ 이 천장은 2026-09-19 첫 실계산과
**정확히 일치했다**: 조류 70% · 바람 60% · 파도 83%. 바람을 막은 것은 데이터가 아니라 설정이었다.)

| 기둥 | 무엇이 막고 있었나 | 넘길 수 있나 |
|---|---|---|
| 금리 감내력 | 선행 EPS 수익률 격차 25%p — **무료 공개 출처가 없다** | ❌ 그 자리를 못 채우면 나머지를 다 채워도 60%에 못 닿는다 |
| 달러 네트워크 | SWIFT 결제비중·BIS 달러신용 30%p — 공개 시계열 없음 | ❌ 스테이블코인(15%p)을 붙여도 57%에서 멈춘다 |
| **재정 우위** | 재정수지·국채 순공급·이자지출 **60%p 전부 미수집** | ⭐ 셋 다 무료 출처가 있다 |

⚠ **재정 우위는 셋을 다 채워야 넘어간다.** 둘만 채우면 50~57%로 게이트에 못 닿는다 —
저빈도 지표는 바람 축의 **분모에서 빠지므로**, 남은 자리가 못 채우는 자리면 비율이 오르지 않는다.

**붙인 것** (`lib/macro/sectors/fiscal.ts` — 새 묶음 「재정·국채」)

| 자리 | 계열 | 출처 |
|---|---|---|
| 적자 압력 25%p | 연방 지출 ÷ 세입 (%) | FRED `FGEXPND` ÷ `FGRECPT` (BEA 국민계정) |
| 국채 순공급 20%p | 시장성 국채 잔액 전년비 | 재무부 MSPD 표 1 — **이미 받고 있었다**(비중 계산의 분모) |
| 이자지출 15%p | 연방 이자지출 ÷ 세입 (%) | FRED `A091RC1Q027SBEA` ÷ `FGRECPT` |

⚠ **금액이 아니라 비율로 붙였다.** 명목 금액은 물가·경제 규모를 따라 커져 백분위가 늘 최악에 붙는다 —
움직이지 않는 지표는 기둥에 아무것도 보태지 않는다(P9 민감도가 찾은 「죽은 임계」와 같은 종류다).
실측으로 확인하고 넣었다: 지출/세입 91~252%(2000년 흑자 ~ 2020년 팬데믹), 이자/세입 12.4~27.6%이고
**1990년대 초가 지금보다 높다**. 두 계열 모두 최근값이 73~76 백분위로, 끝에 붙어 있지 않다.
...
파도    83%       83%     (재정 우위는 파도에 참여하지 않는다)
총점    65%   →   75%     ≥ 70% → 레짐 전이가 열린다
```

### ⚠ 새로 보인 것 — 커버리지는 「두께」가 아니다 (2026-09-20)

재정 우위의 **바람 커버리지가 23% → 100%**가 됐다. 그런데 그 100%는 일간 지표 **둘**
(텀 프리미엄 15%p · 해외 역레포 5%p)에 얹혀 있다. 분기·월간 지표가 §2-4에 따라 **분모에서 빠져**
분모가 0.20으로 줄었기 때문이다. 규칙대로지만, 읽는 사람에게 100%는 「빈틈없다」로 읽힌다.

⚠ **신뢰도(§2-7)가 이것을 잡지 못한다** — `confidence`는 커버리지·신선도·근거·채널폭만 본다.
쓴 지표의 **수(depth)**는 계산되어 있는데 신뢰도 식에 들어가지 않는다.
**숙제로 남긴다**: P9에서 가중치를 만질 때 `depth`를 신뢰도에 넣을지 함께 정한다.
지금 당장 바꾸지 않는 이유는, 신뢰도 식을 고치면 이미 낸 점수의 뜻이 달라지기 때문이다.

### ★ P1 실측 — 첫 종합 점수 (2026-09-20 배포·수집 뒤)

배포하고 수집한 뒤 운영 D1을 **읽기만** 해서 다시 쟀다(`npm run gcrm -- measure`).

```text
         천장 예측        실측 (2026-09-19 기준)
조류      80%       →     41.0  커버리지 80%  두께 64%  기둥 8/10
바람      70%       →     30.9  커버리지 70%  두께 55%  기둥 7/10
파도      83%       →     36.2  커버리지 83%  두께 63%  기둥 5/6
종합                →     36.8  ·  RTE 35.1  ·  레짐 R0
```

⭐ **설정만으로 잰 천장이 실측과 정확히 같았다** — 54에 이어 두 번째다.
설정을 읽으면 데이터를 받기 전에 커버리지를 알 수 있다. 이것은 이 설계의 성질이다:
커버리지는 **데이터가 아니라 설정이 정한다.**

### ⚠ 정정 — 「재정 우위는 셋을 다 채워야 넘어간다」는 틀렸다 (2026-09-20)

위 P1 절에 적은 「둘만 채우면 50~57%」는 **산수가 틀렸다.** 실제로는 **80%**다 —
적자 25%p + 이자 15%p + 이미 있던 40%p. 기둥 게이트 60%를 **둘만으로 넘는다.**
실측이 그대로 보여 줬다(재무부 계열이 아직 0행이던 중간 측정에서 재정 우위 반영률 **80%**,
채운 뒤 **100%**).

⚠ 더 중요한 것: `treasury_marketable`은 `freq: "m"`이고 `axesFor("m") = ["tide"]`라
**바람·파도에는 애초에 들어가지 않는다.** 바람 70%를 연 것은 **FRED 비율 둘**이고,
재무부 계열이 올린 것은 **조류의 두께뿐**이다(62% → 64%).

⭐ **원칙** — 지표를 기둥에 붙이기 전에 `axesFor(freq)`를 먼저 본다.
저빈도 지표는 아무리 중요해도 **조류만** 바꾼다. 「이 지표를 넣으면 바람이 열린다」고 말하려면
그 지표가 바람에 **참여하는지**부터 확인해야 한다.

### ⚠ 수집기 둘의 역사 시작점이 다르다 (2026-09-20 실측에서 발견)

| 수집기 | 어디서 도나 | `HISTORY_START` |
|---|---|---|
| `lib/macro/ingest.ts` | 워커 (cron·관리자 버튼) | **1990-01-01** |
| `scripts/treasury-daily.mjs` | GitHub Actions (워커에서 treasury.gov가 막힌다) | **올해 − 12년** (지금 2014) |

같은 묶음(재정·국채)의 세 계열이 **서로 다른 길이의 역사로 백분위를 잰다.**
정규화 창의 최소 조건(`minObs` 36 · 5년)은 넘으므로 점수는 나오지만,
「1990년 이후 몇 백분위」와 「2014년 이후 몇 백분위」는 **같은 자가 아니다**(§2-3 expanding 창).

### ✅ 해결 (2026-09-24) — 그리고 **문제가 재무부보다 넓었다**

파 보니 진단이 좁았다. 전말은 `docs/분석_정규화창_역사시작점.md`, 요점만 적는다.

**① 원래 처방 「1990으로 통일」은 불가능하다.** 재무부 Fiscal Data를 직접 조회했다 —
MSPD 표1은 **2001-01-31이 첫 기록**이다(입찰은 1979년까지 있다). 1990은 없는 자료다.

**② 진짜 문제는 `max_window` 하나를 72개 지표에 똑같이 붙여 둔 것이었다.**
`normalize.ts`는 **관측 수**로 자르는데 빈도가 다르므로 뜻이 갈렸다 —
일간 10년 · 주간 48년 · 월간 208년 · 분기 625년. **일간 24개만 잘리고 나머지 42개는 안 잘렸다.**
같은 기둥 안에 「최근 10년 백분위」와 「가진 역사 전부의 백분위」가 섞여 있었다.
⚠ 재무부의 2014년은 그 **증상 하나**였다.

**③ 운영자 결정 — 창을 20년으로 통일하고, 못 채운 것은 신뢰도가 말한다.**

| 무엇 | 전 | 후 |
|---|---|---|
| `max_window` | 전부 2500 | `WINDOW_OBS` — 일간 5000 · 주간 1040 · 월간 240 · 분기 80 (**다 20년**) |
| 재무부 수집 시작 | 올해 − 12년(2014) | **2001-01-01 고정** (MSPD 한계) |
| 신뢰도 | 넷 | **다섯 — `depth` 추가**(§2-7) |

⭐ **20년인 이유** — 2008년과 2020년이 둘 다 분포에 들어온다. 10년(2016~)이면 금리 사이클이
하나뿐이라 「극단이 무엇인가」를 정할 수 없다. ⚠ 20이라는 수 자체는 근거가 없다(출처 대장 등급 D).

⭐ **연도를 계산하던 자리를 없앴다** — `올해 − 12년`은 해가 바뀔 때마다 역사의 앞을 **조용히** 잘라
백분위의 자를 매년 바꿨다. 커밋 없이 일어나는 변화다.

⚠ **대가** — 일간 24개의 백분위가 달라져 **첫 종합 점수 36.8과 비교할 수 없다.**
축 이력 나흘치(9/21~24)를 버리고 다시 시작한다. ⭐ 바꿀 거면 오늘이 가장 쌌다 —
방향이 나온 뒤(63점)에 바꿨으면 석 달을 버렸다.

⭐ 어느 쪽이든 **지어내지 않는다** — 짧은 역사를 긴 역사인 척하지 않는다.

### ⚠ 레짐은 데이터가 아니라 **시간**을 기다린다

실측에서 레짐이 R0에 머문 것은 자료 부족이 아니다. `measure`가 축 이력·신호를 **비운 채**
계산하기 때문이고(없는 것을 지어내지 않는다), 방향 판정은 조류 **63영업일** lag을 본다.
즉 레짐이 나오려면 축 점수가 **며칠 쌓여야** 하고, 쌓으려면 저장이 시작돼야 한다.
⚠ 저장 경로는 `/api/gcrm/run`(시크릿 헤더)뿐이다 — P8 화면보다 **저장을 먼저 돌려야** 한다.
...
- code: HY_OAS
  ...
  max_window: 5000          # 20년 = WINDOW_OBS.d (2026-09-24. 그 전에는 2500)
  winsor: [0.01, 0.99]
  channels: [CREDIT]
...
confidence = 100 × ( 0.360·coverage
                   + 0.225·mean(staleness_factor)
                   + 0.180·mean(evidence_factor)
                   + 0.135·channel_breadth
                   + 0.100·mean(depth) )          ← 2026-09-24 추가

channel_breadth = min(1, 확인된 서로 다른 채널 수 / 4)
depth           = min(1, 그 지표가 쓴 관측 수 / max_window)
```

⚠ **넷의 비율은 원래 그대로다**(0.40 : 0.25 : 0.20 : 0.15). 다섯째를 넣으면서 0.90배로 줄였을 뿐이고,
테스트가 그 비율을 대조한다. ⚠ **0.10은 근거가 없다** — 출처 대장 `confidence_weights`(등급 D)에
그렇게 적었고 P9 민감도가 흔든다.

⭐ **`depth`가 잡는 것** — 커버리지는 **값이 있으면 켜진 것으로 센다.** 20년 창에 3.2년만 가진 지표도
커버리지에는 온전히 1로 들어간다(2026-09-20에 「바람의 100%가 일간 지표 둘에 얹혀 있던」 일이 같은 종류다).
⚠ 못 채운 지표를 **빼지 않는다** — 빼면 커버리지가 떨어져 게이트에 걸리고, 짧은 자료를 가진 기둥이
통째로 사라진다. **쓰되 신뢰도로 말한다.**
...
**진행** — P0·P2~P7 완료(2026-09-19~20), P9는 민감도만 완료.
**P1 완료**(2026-09-20 배포·수집): 재정 우위 세 자리를 채워 **바람이 게이트에 닿았고 첫 종합 점수 36.8**이 나왔다.
금리 감내력·달러 네트워크는 무료 출처가 없어 **넘길 수 없다는 것을 계산으로 확인했다**(§2-0 P1 절).
다음은 P9 백테스트, 그 다음이 P8 화면이다 — ⚠ 순서를 바꾸지 않는다(§2-21).

⚠ **P9 전에 정해야 할 것**(§2-0에 근거 있음)
1. ~~수집기 둘의 **역사 시작점이 다르다**(1990 vs 2014)~~ → **해결**(2026-09-24).
   창을 20년으로 통일하고 재무부를 2001-01-01로 고정했다. ⚠ 1990 통일은 **자료가 없어 불가능**했다(MSPD 2001~).
2. ~~축 점수 **저장을 먼저 돌려야** 레짐이 나온다~~ → **해결**(2026-09-21 배포, 2026-09-24 확인).
   매일 06:00 수집 끝에 저장된다. 나흘치가 하루 한 줄로 쌓였고 `verify` 137항목 일치.
   ⚠ 레짐은 예고대로 R0다 — 방향은 조류 63행이 쌓여야 나온다.
3. ⚠ **새로 생긴 것**(2026-09-24 확인에서 발견) — 저장이 **매일**(주말 포함) 돌기 시작하면서,
   「영업일」을 전제한 판정 둘이 **달력일**을 세게 됐다. 방향 lag(`pastAxisScore`는 행 수를 센다)과
   최소 체류(`dwellDays + 1`)다. 63행이 13주가 아니라 **9주**가 되고, 최소 체류는 일찍 풀린다.
   ⚠ **백테스트 전에 정한다** — 영업일 달력으로 세든, 주말에 저장하지 않든 한쪽이어야 한다.
   (§2-8의 「영업일」 정의와 Part 2 「시간대·영업일 달력 정의」가 여기에 걸린다.)
