...
⚠ **P9 백테스트 전에 정한다.** 지금 고치면 이미 낸 점수의 뜻이 달라진다.
선택지는 둘 — ① 재무부도 1990년부터 받는다(MSPD가 그만큼 주는지 확인 필요),
② 정규화 창의 시작을 지표마다 기록해 **화면이 그 사실을 말하게** 한다.
...
1. 수집기 둘의 **역사 시작점이 다르다**(1990 vs 2014) — 백분위를 같은 자로 재지 않고 있다. **미해결.**
