...
### 홈은 블록으로 쪼개져 있다 (2026-08-30, Step 1~2)

홈 라우트는 **조립만 한다**(400줄 → 130줄대). 블록은 `features/home/ui/*`,
"어떤 블록을 어떤 순서로 그릴지"는 `lib/home-layout.ts`(순수 함수 + 테스트)가 정한다.

순서: 히어로 → **거시(지금 경제는 어떤 상태인가)** → 운영 원칙 → 홈 글 →
최신 인사이트 + 티스토리 CTA → **계좌 띠** → 투자일지 · 종목 보고서.

⚠ **계좌는 입구가 아니라 증거다**(Step 2, 2026-08-30). 첫 화면에 있던 계좌 카드를
콘텐츠 뒤의 한 줄 띠(`AccountStrip`)로 내렸다. **지우지는 않았다** — 운영지침 §4의
"고유 콘텐츠를 약화시키지 않는다"와 부딪히기 때문이고, 이 판단은 사람이 정했다.
⚠ 자금 흐름 차트는 홈에서 뺐다 — `/portfolio`에 같은 차트가 **이미 있다**(중복 제거).
⚠ 계좌 숫자가 한 줄이라도 나오면 **모의/실계좌 표시는 의무**다. 자리가 줄어도 따라간다.
⚠ **판단과 문구를 라우트에 다시 넣지 않는다.** 넣는 순간 화면을 띄워야만 확인되는 규칙이
다시 생긴다. 남은 단계(Step 3~6)는 `docs/설계_홈_콘텐츠중심_재편.md`에 있다.
