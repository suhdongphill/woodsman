 ...
 * 처음 받을 때의 시작일.
 *
 * ⚠ **2026-09-24에 「올해 − 12년」에서 고정 날짜로 바꿨다.** 워커 수집기(`features/macro/ingest.ts`)는
 *   1990년부터 받는데 이 스크립트만 12년이라, 같은 묶음의 계열이 **서로 다른 길이의 역사로 백분위를 냈다**
 *   (2026-09-20 실측에서 발견 · `docs/분석_정규화창_역사시작점.md`).
 *
 * ⚠ **1990으로 맞출 수는 없다.** 재무부 Fiscal Data의 MSPD 표1은 **2001-01-31이 첫 기록**이다
 *   (2026-09-24 API 직접 조회로 확인). 입찰은 1979년까지 있지만, 창이 20년이므로 둘 다 2001이면 족하다.
 *
 * ⭐ **연도를 계산하지 않고 박아 둔 이유** — 「올해 − N」은 해가 바뀔 때마다 **역사의 앞이 잘려 나간다.**
 *   조용히, 1년에 한 번, 아무 커밋 없이. 백분위의 자가 매년 달라지면 작년 점수와 올해 점수를 비교할 수 없다.
 */
const HISTORY_START = "2001-01-01";
/**
 ...
 * ⚠ **정규화 창(`WINDOW_OBS` 20년)과 같은 길이로 둔다.** 창이 요구하는 것보다 짧게 갖고 있으면
 *   「20년 분포」라고 말해 놓고 그보다 얕은 자로 재게 된다. 지금 D1에 있는 것은 2014년부터(12.7년)라
 *   **다음 실행이 2001년부터 전 기간을 다시 받는다.**
 */
const MIN_HISTORY_DAYS = 20 * 365;
