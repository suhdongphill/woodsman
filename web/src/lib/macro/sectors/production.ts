/**
 * 생산·제조 섹터 — 묶음 정의와 지표를 **이 파일 하나에** 둔다.
 *
 * ⚠ 지표를 더하거나 임계값을 바꿀 때 다른 파일을 건드리지 않는다
 *    (볼트 인수인계 사양서 1-1: 흩어져 있으면 하나 추가에 여러 곳을 고치게 된다).
 *    새 섹터를 만들 때만 `registry.ts`에 한 줄 등록한다.
 */
import { FRED_URL as FRED, type MacroSector } from "../types";

export const sector: MacroSector = {
  group: {
      key: "production",
      name: "생산·제조",
      emoji: "🏭",
      question: "공장이 돌아가고 있나?",
      intro:
        "제조업은 경기 순환의 앞단입니다. 구매관리자지수(PMI)는 기업 구매 담당자에게 직접 물어 만든 지표라 통계보다 빠르고, 50을 넘으면 확장·밑돌면 위축으로 읽습니다.",
      order: 9,
    },
  indicators: [
    {
      key: "ism_mfg",
      name: "ISM 제조업 PMI",
      group: "production",
      source: "MANUAL",
      transform: "level",
      layer: "L6",
      type: "level",
      freq: "m",
      unit: "",
      decimals: 1,
      url: "https://www.ismworld.org/supply-management-news-and-reports/reports/ism-report-on-business/",
      sourceLabel: "수동 입력 · ISM 원보도자료",
      what: "제조업 구매 담당자들에게 지난달보다 나아졌는지 물어 만든 지수입니다. 50이 기준선입니다.",
      why: "통계가 아니라 현장에 직접 물은 것이라 빠르고, 경기 방향을 앞서 보여줍니다.",
      read: "50 위면 확장, 아래면 위축입니다. 45 아래로 내려가면 침체 국면에서 나오던 수준입니다.",
      signal: { op: "lt", warn: 50, alert: 45, rule: "50 미만 위축 · 45 미만 침체 수준" },
      order: 1,
    },
    {
      key: "ism_svc",
      name: "ISM 서비스업 PMI",
      group: "production",
      source: "MANUAL",
      transform: "level",
      layer: "L6",
      type: "level",
      freq: "m",
      unit: "",
      decimals: 1,
      url: "https://www.ismworld.org/supply-management-news-and-reports/reports/ism-report-on-business/",
      sourceLabel: "수동 입력 · ISM 원보도자료",
      what: "서비스업에 같은 방식으로 물어 만든 지수입니다.",
      why: "미국 경제의 대부분은 제조업이 아니라 서비스업입니다. 제조업이 나빠도 서비스가 버티면 경기는 굴러갑니다.",
      read: "제조업이 50 아래인데 서비스가 50 위면, 아직 경기 전체가 꺾인 것은 아니라고 봅니다.",
      order: 2,
    },
    {
      key: "indpro_yoy",
      name: "산업생산 (전년비)",
      group: "production",
      source: "FRED",
      sourceId: "INDPRO",
      transform: "yoy",
      layer: "L3",
      type: "change",
      freq: "m",
      unit: "%",
      decimals: 1,
      url: FRED("INDPRO"),
      sourceLabel: "FRED · INDPRO",
      what: "공장·광산·전력 회사가 실제로 만들어 낸 양이 1년 전보다 얼마나 늘었는지입니다.",
      why: "설문이 아니라 실제 생산량입니다. 심리 지표가 맞았는지 확인하는 자리입니다.",
      read: "마이너스로 내려가면 실물이 실제로 줄고 있다는 뜻입니다.",
      order: 3,
    },
    {
      key: "phil_fed",
      name: "필라델피아 연준 제조업지수",
      group: "production",
      source: "FRED",
      sourceId: "GACDFSA066MSFRBPHI",
      transform: "level",
      layer: "L6",
      type: "level",
      freq: "m",
      unit: "",
      decimals: 1,
      url: FRED("GACDFSA066MSFRBPHI"),
      sourceLabel: "FRED · GACDFSA066MSFRBPHI",
      what: "필라델피아 연준이 관할 지역 제조업체에 묻는 체감 경기입니다. 0이 기준선입니다.",
      why: "ISM보다 먼저 나옵니다. 이번 달 제조업이 어떨지 미리 엿볼 수 있습니다.",
      read: "0 위면 확장, 아래면 위축입니다. 월별 변동이 큰 편이라 흐름으로 봅니다.",
      order: 4,
    },
    /*
     * ── 명목 GDP ─────────────────────────────────────────────────────
     * ⚠ 노동생산성·단위노동비용은 2026-09-14에 「생산성·공급」(`sectors/supply.ts`)으로 옮겼다.
     *   명목 GDP는 **산출 규모** 이야기라 여기 남는다. `capital.ts`의 성장–조달 격차가 이것을 쓴다.
     */
    {
      key: "ngdp_yoy",
      name: "명목 GDP (전년비)",
      group: "production",
      source: "FRED",
      sourceId: "GDP",
      transform: "yoy",
      layer: "L2",
      type: "change",
      freq: "q",
      unit: "%",
      decimals: 2,
      url: FRED("GDP"),
      sourceLabel: "FRED · GDP (원 발표: 미 경제분석국 BEA)",
      what: "물가 상승분을 빼지 않은, 지금 돈 단위로 센 경제 규모가 1년 전보다 얼마나 커졌는지입니다.",
      why: "⚠ **부채를 갚는 힘은 실질이 아니라 명목으로 자랍니다.** 세금도 이자도 명목으로 내기 때문입니다. 이 값이 국채 금리보다 높으면 경제가 이자보다 빨리 자라고 있다는 뜻이고, 그래서 정부 부채가 굴러갑니다.",
      read: "10년 국채 금리와 견줘 보세요(성장–조달 격차). ⚠ 다만 이것은 **부채 지속가능성** 질문이고, 민간이 빌려서 투자할 값이 있는지는 **다른 질문**입니다 — 둘은 반대로 나올 수 있습니다.",
      order: 5,
    },
    {
      // ⚠ 선행 EPS가 아니다. 무료 공개 출처가 없어 **이미 실현된** 법인이익(NIPA)으로 대신한다 — 이름에 그렇게 적는다.
      // 십억 달러 연율 수준 계열(분기). 전년비는 읽을 때 계산한다.
      key: "corp_profits_yoy",
      name: "기업이익 (실현 · 세후, 전년비)",
      group: "production",
      source: "FRED",
      sourceId: "CP",
      transform: "yoy",
      layer: "L3",
      type: "change",
      freq: "q",
      unit: "%",
      decimals: 1,
      url: FRED("CP"),
      sourceLabel: "FRED · CP (원 발표: 미 경제분석국 BEA 국민계정 · 재고평가·자본소비 조정 전)",
      what: "미국 기업 전체가 세금을 내고 남긴 이익이 1년 전보다 얼마나 늘었는지입니다. 이미 벌어들인 이익이지, 앞으로의 전망이 아닙니다.",
      why: "⭐ **금리를 감당하는 힘의 한 축**입니다. 금리가 올라도 기업이익이 그보다 빨리 늘면 시장은 새 금리에 적응할 수 있습니다(명세 Rate Absorption). ⚠ 명세는 **선행 EPS 성장**을 요구하지만 무료 출처가 없어, 이 값을 **「기업이익(실현)」이라는 이름으로** 대신 씁니다.",
      read: "⚠ 분기 발표라 금리보다 두세 달 늦고, 발표 뒤 크게 고쳐집니다(속보 → 잠정 → 확정). 한 분기보다 추세를 보세요. 주가가 보는 것은 앞으로의 이익이라, 이 값과 주가가 엇갈리는 구간이 흔합니다.",
      order: 6,
    },
    {
      // 지수(2017=100) 수준 계열(월간·계절조정).
      key: "power_ip_yoy",
      name: "전력 생산·송전 (전년비)",
      group: "production",
      source: "FRED",
      sourceId: "IPG2211S",
      transform: "yoy",
      layer: "L3",
      type: "change",
      freq: "m",
      unit: "%",
      decimals: 1,
      url: FRED("IPG2211S"),
      sourceLabel: "FRED · IPG2211S (원 발표: 연준 G.17 산업생산)",
      what: "미국의 전력 생산·송전·배전 활동이 1년 전보다 얼마나 늘었는지입니다.",
      why: "데이터센터는 전기를 먹습니다. AI 투자가 실제로 돌아가기 시작하면 전력 수요가 먼저 움직입니다. 명세 AI Demand의 「Power/Grid Demand」 자리입니다.",
      read: "⚠ **AI 전용 수요가 아니라 전체 전력**입니다. 여름이 덥거나 겨울이 추우면 날씨만으로도 크게 움직입니다 — 계절조정이 되어 있어도 이상기후는 남습니다. 몇 달 연속으로 추세가 바뀌는지를 보세요.",
      order: 7,
    },
  ],
};
