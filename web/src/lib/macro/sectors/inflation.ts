  ...
  indicators: [
    {
      ...
      decimals: 1,
      url: FRED("CPIAUCSL"),
      sourceLabel: "FRED · CPIAUCSL (계절조정)",
      what: "가계가 사는 물건과 서비스 값이 1년 전보다 얼마나 올랐는지입니다. ⚠ 계절조정(SA) 계열이라 BLS 보도자료의 전년비(비계절조정)와 0.1%p 안팎 다를 수 있습니다.",
      why: "연준의 목표는 2%입니다. 여기서 멀어질수록 금리 정책이 흔들리고, 그 여파가 주식·채권 전부에 옵니다.",
      /**
       * ⚠ 2026-09-07 확인. 대시보드 3.3% vs BLS 원문 3.4%는 **둘 다 맞다** —
       * 우리는 `CPIAUCSL`(계절조정)의 전년비를 쓰고, BLS 보도자료의 전년비는
       * `CPIAUCNS`(비계절조정) 기준이다. 인용할 때 어느 쪽인지 밝히면 충돌이 없다.
       * 계열을 바꾸지 않은 이유: 월별 흐름을 볼 때는 계절조정이 맞고, 바꾸면 쌓은 시계열이 끊긴다.
       */
      read: "2%에 가까워지면 금리를 내릴 여지가 생기고, 다시 튀어 오르면 인하 기대가 사라집니다. ⚠ 글에 인용할 때는 「계절조정 기준」임을 함께 적습니다 — BLS 보도자료 숫자와 소수점이 다릅니다.",
      headline: true,
      order: 1,
    },
    {
    ...
    },
    {
      ...
      freq: "m",
      /**
       * ⚠ 2026-08-22 실측 보정. 기본 75일로 재니 **정상인데 기한초과로 떴다.**
       *   BEA 개인소득·지출은 기준월 종료 후 약 4주 뒤 발표한다(7월분이 8월 말).
       *   그날 FRED 원본의 최신 관측도 `2026-06-01`이었다 — 우리 수집이 늦은 게 아니다.
       *   ⚠ 볼트 대시보드에는 아직 이 예외가 없다. 그쪽도 같이 손봐야 한다.
       */
      staleDays: 95,
      staleWhy:
        "BEA 개인소득·지출(Core PCE)은 기준월 종료 후 약 4주 뒤 발표된다(7월분이 8월 말). 기본 75일로 재면 다음 발표 전에 만료된다.",
      unit: "%",
      decimals: 1,
      url: FRED("PCEPILFE"),
