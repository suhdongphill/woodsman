...
export function dataModeNotice(mode: DataMode): DataModeNotice {
  /**
   * ⚠ 이름은 「실계좌 공개」가 아니라 **「운영 포트폴리오」**다(2026-09-25 사용자 결정).
   *   증권사 잔고를 그대로 싣고 금액도 공개하되(결정 B), 종목은 관리자가 공개를 켠 것만 보인다.
   *   ⚠ 그래서 문구에 "평가액은 계좌 전체, 종목은 고른 것만"을 함께 적는다 — 안 적으면
   *   보이는 종목의 합과 평가액이 어긋나 숫자를 의심받는다.
   *   ⚠ 실돈을 「모의 투자」로 부르지 않는다 — PAPER 문구는 "매매와 납입은 가상"이라 사실과 다르다.
   */
  if (mode === "LIVE") {
    return {
      badge: "운영 포트폴리오",
      line: "실제 자금으로 운용 중인 포트폴리오입니다. 매매·납입·평가액 모두 실제이며, 평가액은 계좌 전체 기준이고 종목은 공개를 고른 것만 보입니다.",
      isRealMoney: true,
      tone: "ok",
    };
  }
