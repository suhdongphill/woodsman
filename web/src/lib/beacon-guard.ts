 ...
 *
 * ## ⚠ 속도 제한의 정체 (2026-08-11 실측 → 2026-08-15 정정)
 * 처음에는 Cloudflare Rate Limiting 바인딩(`BEACON_LIMITER`, 60회/분)을 **"실제 방어선"**
 * 이라고 적었다가, 8/11 실측에서 447건 전부 통과하는 것을 보고 **"아예 동작하지 않는다"**
 * 고 정정했다. 8/15에 배포본에서 다시 재 보니 **그 정정도 정확하지 않았다.**
 *
 * | 방식 | 결과 |
 * |---|---|
 * | 순차 100건 / 29초 (`view`) | **32건 차단** — 막는다 |
 * | 병렬 120건 / 18초 (`engagement`) | **0건 차단** |
 * | 그 직후 순차 30건 (같은 키) | **0건 차단** — 폭주분이 카운터에 아예 안 잡혔다 |
 *
 * 즉 **한 흐름으로 꾸준히 두드리면 막고, 동시에 퍼뜨리면 통째로 빠져나간다.**
 * (가설: 동시 요청이 여러 isolate로 흩어지면 각자의 지역 카운터가 상한에 못 미친다.
 * Cloudflare 문서의 *permissive · eventually consistent* 가 이 모습일 것이다.)
 * ⚠ **남용은 정확히 뒤쪽 방식으로 온다.** 그래서 이것을 방어선으로 인정하지 않는다.
 *
 * ## ⚠ 원인 가설 하나는 확실히 배제됐다 — OpenNext env 프록시가 아니다
 ...
 * | 3 | **볼륨(같은 경로 부풀리기)** | `BEACON_LIMITER` | ⚠ **꾸준한 남용만 막는다. 폭주는 못 막는다** |
 *
 * ⚠ 3층이 반만 서 있는 동안 초기 통계는 **덜 믿어야 한다.** 후속안은
 * `docs/계획_2026-08-15.md` §3에 적었다.
 *
 * ## ⚠ 조용히 실패하지 않는다
