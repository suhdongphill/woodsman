  ...
  if (probe.blocked === 0) {
    return {
      ...
      detail:
        ...
        `첫 반환값은 ${probe.firstResult ?? "확인 불가"}였습니다. 2026-08-11 운영 실측과 같은 증상입니다.`,
    };
  }
  return {
    level: "ok",
    headline: "막습니다",
    detail:
      `${probe.calls}회 중 ${probe.blocked}회가 차단됐습니다(상한 ${expectedLimit}회). ` +
      `기대 차단 수는 약 ${probe.calls - expectedLimit}회입니다 — Cloudflare 속도 제한은 정확한 계수 장치가 아니라 오차가 있습니다.`,
  };
}
